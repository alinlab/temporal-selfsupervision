...
def spatial_sampling(
    ...
    random_samples = {'random_short_side_scale_jitter': {}, 'random_crop': {}, 'horizontal_flip': {}}

    if spatial_idx == -1:
        frames, _, size = transform.random_short_side_scale_jitter(
            images=frames,
            min_size=min_scale,
            max_size=max_scale,
            inverse_uniform_sampling=inverse_uniform_sampling,
        )
        random_samples['random_short_side_scale_jitter']['size'] = size


        frames, _, x_offset, y_offset = transform.random_crop(frames, crop_size)
        random_samples['random_crop']['x_offset'] = x_offset
        random_samples['random_crop']['y_offset'] = y_offset

        if random_horizontal_flip:
            frames, _, sample = transform.horizontal_flip(0.5, frames)
            random_samples['random_crop']['sample'] = sample
    else:
        # The testing is deterministic and no jitter should be performed.
        # min_scale, max_scale, and crop_size are expect to be the same.
        #assert len({min_scale, max_scale, crop_size}) == 1
        frames, _, _ = transform.random_short_side_scale_jitter(
            frames, min_scale, max_scale
        )
        frames, _ = transform.uniform_crop(frames, crop_size, spatial_idx)
    
    if return_random_samples:
        outputs = (frames, random_samples)
    else:
        outputs = frames
...
def tensor_normalize(tensor, mean=None, std=None):
    ...
    if mean is not None:
        assert isinstance(mean, (float, int, list)), f"mean is not supported dtype. Recieved {mean} of type {type(mean)}"
        if type(mean) == list:
            mean = torch.tensor(mean)
        tensor = tensor - mean

    if std is not None:
        assert isinstance(std, (float, int, list)), f"std is not supported dtype. Recieved {std} of type {type(std)}"
        if type(std) == list:
