...
def _get_model_analysis_input(cfg, use_train_input):
    ...
    if model_name in ['timesformer_vit_base_patch16_224']:
        if use_train_input:
            input = torch.rand(
                cfg.DATA.NUM_FRAMES,
                rgb_dimension,
    ...
    if cfg.NUM_GPUS:
        ...
        for i in range(len(input_tensors)):
            input_tensors[i] = input_tensors[i].cuda(device=cur_device, non_blocking=True).unsqueeze(0)
...
def is_eval_epoch(cfg, cur_epoch, multigrid_schedule):
    ...
    return (cur_epoch + 1) % cfg.ERM_TRAIN.EVAL_PERIOD == 0
