...
def is_checkpoint_epoch(cfg, cur_epoch, multigrid_schedule=None):
    ...
    return (cur_epoch + 1) % cfg.ERM_TRAIN.CHECKPOINT_PERIOD == 0


def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, cur_iter=0, path_to_checkpoint=None):
    """
    Save a checkpoint.
    Args:
...
#        if True:
    ...
    if inflation:
        ...
        # pre_train_dict = {k.replace("model.", ""): v for k, v in pre_train_dict.items()}
        model_dict = ms.state_dict()

        for k in list(pre_train_dict.keys()):
            if 'blocks' in k and 'attn' in k:
                new_key = k.replace('attn','temporal_attn')
                if not new_key in pre_train_dict:
                    pre_train_dict[new_key] = pre_train_dict[k]
            elif 'blocks' in k and 'norm1' in k:
                new_key = k.replace('norm1','temporal_norm1')
                if not new_key in pre_train_dict:
                    pre_train_dict[new_key] = pre_train_dict[k]
            elif k == 'time_embed' and pre_train_dict[k].shape != model_dict[k].shape:
                new_v = F.interpolate(pre_train_dict[k].transpose(1,2), size=(model_dict[k].size(1)), mode='nearest')
...
def load_test_checkpoint(cfg, model):
    ...
    elif cfg.ERM_TRAIN.CHECKPOINT_FILE_PATH != "":
        # If no checkpoint found in TEST.CHECKPOINT_FILE_PATH or in the current
        # checkpoint folder, try to load checkpoint from
        # TRAIN.CHECKPOINT_FILE_PATH and test it.
        load_checkpoint(
            cfg.ERM_TRAIN.CHECKPOINT_FILE_PATH,
            model,
            cfg.NUM_GPUS > 1,
            None
        )
...
def load_train_checkpoint(cfg, model, optimizer):
    ...
    """
    if cfg.ERM_TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR)
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            cfg.NUM_GPUS > 1,
            optimizer,
            epoch_reset=cfg.ERM_TRAIN.CHECKPOINT_EPOCH_RESET,
            clear_name_pattern=cfg.ERM_TRAIN.CHECKPOINT_CLEAR_NAME_PATTERN,
        )
        start_epoch = checkpoint_epoch + 1
    else:
        start_epoch = 0
